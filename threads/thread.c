...
bool thread_mlfqs;; // MLFQS 스케줄러 사용 여부 (커널 명령줄 옵션으로 제어)
...
thread_init (void) {
	...
	struct desc_ptr gdt_ds = { // 임시 GDT 디스크립터
...
thread_launch (struct thread *th) {
	...
	__asm __volatile (
			...
			); // 현재 스레드의 레지스터 값을 저장하고 새 스레드의 컨텍스트로 전환
}
...
do_schedule(int status) {
	...
	while (!list_empty (&destruction_req)) { // 파괴 요청 리스트가 비어있지 않으면
...
schedule (void) {
	...
	if (curr != next) { // 현재 스레드와 다음 스레드가 다르면 (스레드 전환 필요)
		/* If the thread we switched from is dying, destroy its struct
		   ...
		   schedule(). */
		if (curr && curr->status == THREAD_DYING && curr != initial_thread) {  // 종료 중인 스레드가 있고 초기 스레드가 아니면
...
void thread_sleep(int64_t ticks) {
	...
	if (curr != idle_thread){ // 현재 스레드가 아이들 스레드가 아니면
		...
		if (min_wakeup_tick > ticks) min_wakeup_tick = ticks; // 글로벌 틱 업데이트
	}
